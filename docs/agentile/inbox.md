...
- [ ] Content-aware language detection: a python snippet selected inside a .md file is snapped as markdown because detection is filename/shebang only (lib/language.mjs); sniff the selected text (keywords, or tree-sitter error ratio across candidate grammars) and let a confident sniff beat a weak filename match like .md/.txt/no filename. The preview override (language button bottom-left, Left/Right to cycle) already shipped in 0006 — (captured 2026-09-07)
