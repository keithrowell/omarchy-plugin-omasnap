 ...
 *
 * `wrap` (default `true`) reflows every line to `wrapWidth` characters
 * (default `DEFAULT_WRAP_WIDTH`) via `lib/wrap.mjs`, *after* tab expansion
 * so a tab counts as the columns it actually renders, not one character —
 * `snap.lineNumbers` (same length as `snap.lines`, `null` on a
 * continuation row) is what `app/Snap.qml`'s gutter reads instead of just
 * numbering every rendered row 1, 2, 3...
 */
export function buildInput({ snap, theme, look, headerFont, wallpaperOverride, wrap = true, wrapWidth = DEFAULT_WRAP_WIDTH } = {}) {
  ...
  const resolvedSpans = resolveSpans(snap.lines, theme);
  const { lines, lineNumbers } = wrap
    ? wrapLines(resolvedSpans, wrapWidth)
    : { lines: resolvedSpans, lineNumbers: resolvedSpans.map((_, i) => i + 1) };
...
function parseArgs(argv) {
  const args = { fixture: null, themeDir: DEFAULT_THEME_DIR, wallpaper: undefined, wrap: true, wrapWidth: DEFAULT_WRAP_WIDTH };
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (arg === "--wrap") args.wrap = true;
    else if (arg === "--no-wrap") args.wrap = false;
    else if (arg === "--wrap-width") args.wrapWidth = Number(argv[++i]);
...
function main(argv) {
  ...
  const input = buildInput({ snap, theme, wallpaperOverride: args.wallpaper, wrap: args.wrap, wrapWidth: args.wrapWidth });
  console.log(JSON.stringify(input));
}

// CLI entry point: `node lib/input.mjs --fixture F [--theme-dir D] [--wallpaper W] [--wrap|--no-wrap] [--wrap-width N]`.
