...
import { buildInput, DEFAULT_WRAP_WIDTH } from "./input.mjs";
...
function writeInput({ outPath, snap, warnings, detected, theme, wrap, wrapWidth }) {
  const input = buildInput({ snap, theme, wrap, wrapWidth });
...
async function main(argv) {
  ...
  if (args.request && !args.selection && !args.window) {
    ...
    try {
      ...
      writeInput({ outPath: args.out, snap: result.snap, warnings: result.warnings, detected: result.detected, theme, wrap: args.wrap, wrapWidth: args.wrapWidth });
    } catch (err) {
      console.error(`snap: ${err.message}`);
      process.exit(1);
    }
